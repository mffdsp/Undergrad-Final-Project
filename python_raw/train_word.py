...
max_iters = 6000  # Epochs
eval_interval = 250  # Estimate loss at each N sample of epochs
...
class Head(nn.Module):
    ...
    def forward(self, x):
        ...
        B, T, C = x.shape
        key_ = self.key(x)  # (B,T,hs)
        query_ = self.query(x)  # (B,T,hs)

        ## SELF_ATTENTION
        # Todo token vai em cada posição irá emitir dois vetores, query e key.
        # Fazemos então o produto escalar entre os vetores: T: Query(A) * Key(T - A)
        # compute attention scores ("affinities")
        wei = (
            query_ @ key_.transpose(-2, -1) * key_.shape[-1] ** -0.5
        )  # (B, T, hs) @ (B, hs, T) -> (B, T, T)

        ## Prevent nodes from the future to 'talk' to the past
